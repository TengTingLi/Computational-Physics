 ...
 * @date 7-6-2023
 */
public class quantum_mechanic_1D extends javax.swing.JFrame {

    /**
     * Creates new form wave_equation
     */
    public quantum_mechanic_1D() {
        initComponents();
    }
    ...
    */
    ...
    public void draw_axis(String x_label,String y_label){
        ...
        gg.drawString(x_label, p_panel.getWidth() - y_pixel_length, offsetj - 8);
...
// </editor-fold>
    ...
    double psi[] = new double[2000]; // wf
    //square well for -x_max to max
    double dx;
    double V;
    double E;
    double dE;
    ...
    public void calculate(double V_max){
        psi[0] = 1; //search for even parity solution
        psi[1] = 1;
        int diverge = 0;
        int last_diverge = 0;   //keep track of direction of last divergence
        int i;
            for(double j = 0; j < 50; j++){
                for(i = 1; i < psi.length-1; i++){
                    psi[i+1] = (- (E - V(i*dx)) * dx * dx + 2) * psi[i] - psi[i-1];
                    if(Math.abs(psi[i+1]) > 2){
                        break;
                    }
                }
            if(psi[i+1] > 0){
    ...
    public double V(double x){
        //lol ternary operator in returb
        return Math.abs(x) <= 1 ? 0 : V;
    }
    ...
    public void display(double psi[]){
        set_pen_color(new Color(R(),R(),R()));
        for(int i = 0; i < psi.length; i++){
            plot_point(i*dx, psi[i]);
            plot_point(-i*dx, psi[i]);
        }
    }
    ...
    private void jButton1ActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_jButton1ActionPerformed
        draw(panel_plot);
        init_plot(0,0,3,2,50,75);
        draw_axis("distance", "Energy");
        set_pen_size(2);
        initialize();
        draw_string(String.format("Potential ouside the well = %.0f", V), panel_plot.getWidth()/4,100);
        calculate(V);
        draw_string(String.format("Energy = %.2f", E), panel_plot.getWidth()/4,90);
    ...
    public static void main(String args[]) {
        ...
        try {
            ...
            java.util.logging.Logger.getLogger(quantum_mechanic_1D.class.getName()).log(java.util.logging.Level.SEVERE, null, ex);
        ...
        java.awt.EventQueue.invokeLater(new Runnable() {
            public void run() {
                new quantum_mechanic_1D().setVisible(true);
            }
        });
    }
